...
export const setTwoPanelLayoutHeight = () => {
  setTimeout(() => {
    ...
    if (bottomnav && ele) {
      ...
      const eley = ele?.getBoundingClientRect().y || 0;
      // height: calc(100vh - ${eley}px)
      ele?.setAttribute(
        'style',
        `
        height: calc(100vh - ${eley < 0 ? 0 : eley}px)
      `,
      );
    }
  }, 0);
...
export function Layout({
  ...
  const ismanagedAccountSubscribers = matchPath(history.location.pathname, {
    path: '/managed-accounts/:id/subscribers/:subscriberId',
    exact: true,
    strict: false,
  });
  const isMember = matchPath(history.location.pathname, {
    path: '/my-members',
    exact: true,
    strict: false,
  });
  const isMemberContent = matchPath(history.location.pathname, {
    path: '/my-members/members-content',
    exact: true,
    strict: false,
  });
  ...
  const isMessages = matchPath(history.location.pathname, {
    path: '/messages/subscribers',
    exact: true,
    strict: false,
  });
  const isSubscriptions = matchPath(history.location.pathname, {
    path: '/messages/subscriptions',
    exact: true,
    strict: false,
  });
  const managedAccountSubs = matchPath(history.location.pathname, {
    path: '/managed-accounts/:id/subscribers',
    exact: true,
    strict: false,
  });
  const managedAccountId = matchPath(history.location.pathname, {
    path: '/managed-accounts/:id',
    exact: true,
    strict: false,
  });
  const isVault = matchPath(history.location.pathname, {
    path: '/vault',
    exact: true,
    strict: false,
  });
  ...
  const handleLeftScroll = async (event: any) => {
    if (leftPanelRef.current && leftSCrollbarRef.current) {
  ...
  const handleRightScroll = async (event: any) => {
    if (leftPanelRef.current && rightScrollbarRef.current) {
      const { scrollTop, scrollHeight, clientHeight } =
          ...
          <AnimatePresence initial={false}>
            {activeView === 'left' || rendering.current ? (
              <div
                key="1"
                style={{ height: '100%' }}
                className="animated-block-parent"
              >
